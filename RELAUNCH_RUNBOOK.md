...
| Pre-compute / vanity-mine a Launchpad collection or NFT pool address off-chain (Wave-2 2026-05-20) | The CREATE2 salt now uses a **per-creator nonce**, not the global `allCollections.length` / `_allPools.length`. Any off-chain tool that pre-computes the deterministic address (vanity mining, pre-funding, off-chain coordination) MUST read `TegridyLaunchpadV2.collectionsCreatedByCreator(creator)` (resp. `TegridyNFTPoolFactory.poolsCreatedByCreator(creator)`) for the current nonce and use it as the salt component, NOT the array length. Salt format is unchanged otherwise: `keccak256(abi.encode(chainid, address(factory), msg.sender, nonce, ...))` for Launchpad and `keccak256(abi.encodePacked(chainid, factory, msg.sender, nonce, nftCollection, uint8(poolType)))` for NFTPoolFactory. The change closes a bounded grief where another creator's deploy could shift a victim's pending deterministic address by advancing the global counter. |
