...
// AUDIT FIX 2026-05-26 [H-24] DEFERRED: ERC-721 Transfer subscription would
// require the TegridyStakingAbi auto-generated by ponder.config.ts to include
// the ERC-721 Transfer(address indexed from, address indexed to, uint256
// indexed tokenId) event — currently the Solady ERC721 base used by
// TegridyStaking does NOT surface it through the generated event union.
// Fix: extend ponder.config.ts's `events` filter to include a manual
// Transfer fragment OR migrate to OZ ERC-721 (which is in the ABI by
// default). Tracked for a dedicated indexer-schema-extension PR; current
// state means "your positions" can still misattribute after secondary
// market trades. Mitigation: frontend pulls ownerOf() on-chain for any
// position row whose user no longer matches the wallet.
