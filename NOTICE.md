...
| **Raydium CLMM concentrated-liquidity math** | [raydium-io/raydium-clmm](https://github.com/raydium-io/raydium-clmm) @ `bf7c241` | **Apache-2.0** | [`solana/tegridy-amm/programs/tegridy-launch/src/vendor/`](solana/tegridy-amm/programs/tegridy-launch/src/vendor/) vendors seven library files (`big_num`, `fixed_point_64`, `full_math`, `liquidity_math`, `sqrt_price_math`, `tick_math`, `unsafe_math`) to price the segmented bonding curve. **Changes made (Apache-2.0 §4(b)):** import paths rewritten to the local module, plus deletion of the unused `construct_bignum!` macro and `U1024` from `big_num.rs`. **No formula, rounding direction or bound is altered.** The delta is enforced by the `clmm-vendor-guard` job in `.github/workflows/solana-ci.yml`. Licence verified by reading upstream `LICENSE`, not API metadata — see [`docs/CURVE_FORK_EVALUATION.md`](docs/CURVE_FORK_EVALUATION.md). **Upstream's audits do NOT cover this vendored copy.** |
