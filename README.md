...
5. A Solana surface earns fees too — the Jupiter-routed swap is live, and the Meteora launcher rail is **armed on mainnet (partner config live 2026-08-01) but has launched zero tokens**. Its page **now has an in-app submit path** (shipped in #259, 2026-08-04): a member of the public signs and submits their own launch, gated on a published partner config — with no config published for a given build, the page falls back to a config preview and says so. *(Corrected 2026-08-07: this line, and the two below, still claimed "no submit path" long after #259 merged.)* The Tegridy-owned Solana programs were deployed to **mainnet on 2026-08-08 and closed on 2026-08-13**; their program ids are spent and not reusable, so today there is no Tegridy-owned Solana venue. *(Corrected 2026-09-02 — this line said "written but deployed nowhere", which was true when written and stopped being true on 2026-08-08. See the Solana bullet in "What's left" for the ids and how to check them.)*
...
| **Solana launcher** | [/solana-launch](https://memetic.fun/solana-launch) | Meteora DBC | 🟢 **Rail armed — 0 launches so far.** The partner config is live on Solana mainnet (2026-08-01) and the Squads v4 fee vault is verified, so the rail can take a launch. The page **has an in-app submit path** as of #259 (2026-08-04) — the public signs and submits their own launch when `VITE_SOLANA_DBC_CONFIG` names a published config for that build; otherwise it renders a config preview and labels itself PREVIEW. Config *creation* stays out-of-band via the operator CLI ([`frontend/scripts/solana-dbc-operator.mjs`](frontend/scripts/solana-dbc-operator.mjs)), because that is the step requiring the fee claimer to be the derived Squads vault |
| **Own Solana curve** | — | `tegridy-launch` bonding curve → a Tegridy-owned CP-AMM pool | 🔴 **Code on trunk; was on mainnet 2026-08-08 → 2026-08-13, now closed.** Source at [`solana/tegridy-amm/programs/tegridy-launch`](solana/tegridy-amm/programs/tegridy-launch). The program id `6vV7DqMy…` is **spent and not reusable** — a redeploy needs a fresh keypair. Its *ProgramData* account returns `null` at 0 lamports; the program stub still reads `executable: true`, so check ProgramData, not the program account. *(Corrected 2026-09-02 — the earlier "deployed to no cluster … checked 2026-07-31" predated the deploy.)* |
...
- **Solana launcher (`/solana-launch`) — the rail is armed, and the page can now submit.** As of **2026-08-01** a Meteora **DBC partner config** exists on Solana **mainnet** (`create-config` run by the operator) against a verified Squads v4 fee vault, so the rail can accept a launch. **No token has launched through it yet** — every number on the page is a builder default, not observed history. Since **#259 (2026-08-04)** the page **does** mount a signer and submit: a member of the public builds, signs and sends their own launch transaction. It is gated on `isSolanaSubmitReady()` — `SOLANA_LAUNCHER_ENABLED` **and** a `VITE_SOLANA_DBC_CONFIG` published into that build. With no config published the page renders its configuration preview and labels itself PREVIEW, which is the honest state, not a claim that the rail is dead. Config *creation* remains out-of-band via the operator CLI ([`frontend/scripts/solana-dbc-operator.mjs`](frontend/scripts/solana-dbc-operator.mjs)) — that is the step that forces the fee claimer to be the derived Squads vault, and it must never run in a browser.
  > ⚠️ **`VITE_SOLANA_DBC_CONFIG` is a build-time variable.** Vite inlines it, so publishing a config on-chain changes nothing until the site is **rebuilt and redeployed**. If `/solana-launch` says PREVIEW in production, check the deploy before checking the chain.
...
- **Solana — two programs, deployed to mainnet and then CLOSED.** ~~Deployed to no cluster at all… the program id resolves nowhere.~~ **Corrected 2026-09-02 (audit TF-029):** both programs *were* deployed to mainnet on **2026-08-08** and **closed on 2026-08-13** to recover the rent. This matters for anyone reading the ids as fresh: the program ids are **SPENT and cannot be reused** — `tegridy-launch` `6vV7DqMyGwpM18rf2Lkefa1U9YfKquZjvwA61ch3FsnS` and the cp-swap fork `6TnZb1GTHhPAYsrbtwfELkqQrXyqCfv7V6s27RJKXHAF`. Their ProgramData accounts return `null` at 0 lamports on two independent RPCs; the program *stubs* survive and still read `executable: true`, so a naive `getAccountInfo` on the program id looks healthy — **check the ProgramData account, not the program account.** A redeploy needs fresh keypairs. Both hold no funds; path is diff-audit → mainnet → Jupiter integration.
...
- **`LockerClaimer` adoption** — the small contract that lets the launcher's 15% fee line reach TOWELI stakers instead of resting in the treasury. ~~Written and tested, wired to nothing.~~ **Corrected 2026-09-02 (audit TF-009):** that clause contradicted this same README a few hundred lines up. The contract *is* deployed and Etherscan-verified at [`0xD2Ac3dC1…`](https://etherscan.io/address/0xd2ac3dc13c6fd09855f0e4a077826983aa66e6c7#code) (2026-08-01, by hand — which is why there is no broadcast receipt; see `frontend/scripts/addresses.json` and `docs/MIGRATION_HISTORY.md:63`). What is still outstanding is **adoption**: no launch has routed its fee line through it yet.
