...
- 🔴 **The protocol-owned TOWELI/WETH pool is effectively empty, and LP farming is not running.** Verified on-chain 2026-08-02: the native pair holds **146,258 TOWELI + 0.00383 WETH (~$14 total)** after ~83% of its LP was burned, and the LP Farming contract's staked LP balance is **0** (was 125.0). `SwapFeeRouter.totalETHFees()` is **0** — the fee rail has never captured a wei, so no ETH yield epoch has ever opened. Swaps route to the deepest venue (the Uniswap V2 pool, ~1,956× deeper in WETH), so trading is unaffected; what is blocked is the TWAP oracle (floor 10 WETH), and with it TegridyLending, POL accumulation, and native limit-order fills. Deepen + bootstrap remain scripted ([`DeepenLP.s.sol`](contracts/script/DeepenLP.s.sol), [`BootstrapTWAP.s.sol`](contracts/script/BootstrapTWAP.s.sol)), but the old ~1.33 WETH target is **undersized** — independent modelling puts the level where the native pool starts winning $1k+ trades at ~7.5–9 WETH.
...
| TOWELI/WETH pair (native — **drained to ~$14, 2026-08-02**) | [`0x55875…a481`](https://etherscan.io/address/0x55875887B43C2E23aE424AF0FC8606Fdb058a481) |
