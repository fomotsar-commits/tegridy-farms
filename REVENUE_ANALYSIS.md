...
|---|-------|-----------------|--------------|----------|----------|--------------|
| 1 | Swap fee (own DEX path) | `SwapFeeRouter.sol` via `swapExactETHForTokens`/`…ForETH` | **mutable state var** — Swap fee is a mutable state var on `SwapFeeRouter` queryable via `feeBps()` getter; default 50 bps (0.50%) configurable via a 24 h timelock (`proposeFeeChange`) | 1.00 % (`MAX_FEE_BPS = 100`, hard constant) | Swapper | ~80 % → `RevenueDistributor` → stakers (pro-rata TOWELI lock-weighted). **Corrected 2026-09-02 (audit TF-046 / TF-071):** this cell said "100 %" and "48h timelock", contradicting §"Revenue out" and the fee-lever table lower in this same file. `ReferralSplitter` carves `referralFeeBps` (2000) off the top first and it cannot be set to zero (`FEE_CANNOT_BE_ZERO`), so 100 % of what *arrives* is ~80 % of what is *collected*. Nothing has been distributed to date. |
