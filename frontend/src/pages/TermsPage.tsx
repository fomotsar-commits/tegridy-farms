...
export const SECTIONS = [
  ...
  },
  {
    ...
    body: 'Tegridy Farms is a decentralized finance (DeFi) protocol built on the Ethereum blockchain. The Protocol provides the following services: yield farming and liquidity provision through automated market maker pools; token swapping via integrated DEX functionality; TOWELI token staking with vote-escrow mechanics; peer-to-peer NFT lending with collateral-based loan origination; an NFT AMM for automated NFT trading; and a token launcher through which any user may create, issue and deploy a new ERC-20 token on Ethereum mainnet using the third-party Doppler protocol, paired against either native ETH or TOWELI, which runs a dynamic auction and graduates into a Uniswap V4 pool. The Protocol additionally publishes a Solana sub-brand launch rail over Meteora\'s third-party Dynamic Bonding Curve, through which any user may create and launch a new SPL token on Solana mainnet. That rail is non-custodial and user-submitted: you connect your own Solana wallet, you sign the launch transaction yourself, and your wallet pays the network fees; the Protocol never takes custody of the token, its liquidity, or your funds, and cannot submit a launch without your signature. A Solana launch runs against a fixed partner configuration published by the Protocol; that configuration is IMMUTABLE once created, so the bonding curve, the fee schedule (which opens at a high anti-snipe rate and decays over time) and the split of trading fees are fixed before you launch, are displayed to you as read from the on-chain account, and cannot be altered by you or by the Protocol afterwards. A launch is irreversible. All Ethereum services are provided through non-custodial smart contracts deployed on the Ethereum mainnet.',
  },
  {
    title: '3. Eligibility',
