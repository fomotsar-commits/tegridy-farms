...
export default function RestakePage({ embedded }: { embedded?: boolean }) {
  ...
  const { data: restakeData, refetch } = useReadContracts({
    contracts: [
      { address: TEGRIDY_RESTAKING_ADDRESS, abi: TEGRIDY_RESTAKING_ABI, functionName: 'restakers', args: [address ?? '0x0000000000000000000000000000000000000000'] },
      { address: TEGRIDY_RESTAKING_ADDRESS, abi: TEGRIDY_RESTAKING_ABI, functionName: 'pendingBonus', args: [address ?? '0x0000000000000000000000000000000000000000'] },
      { address: TEGRIDY_RESTAKING_ADDRESS, abi: TEGRIDY_RESTAKING_ABI, functionName: 'pendingBase', args: [address ?? '0x0000000000000000000000000000000000000000'] },
    ...
    <div className={embedded ? '' : '-mt-14 relative min-h-screen'}>
      {!embedded && (
        <div className="fixed inset-0 z-0" style={{ background: '#060c1a' }}>
          <img src={ART.beachVibes.src} alt="" loading="lazy" className="w-full h-full object-cover" style={{ objectPosition: 'center 30%' }} />
        </div>
      )}
          ...
          <div className="relative z-10 p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                <div key={s.step} className="rounded-lg p-3" style={{ background: 'rgba(139,92,246,0.75)', border: '1px solid rgba(139,92,246,0.75)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] text-white" style={{ background: 'rgba(139,92,246,0.75)' }}>{s.step}</span>
                    <span className="text-white text-[12px] font-medium">{s.title}</span>
                  </div>
                  <p className="text-white text-[11px]">{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.div className="grid grid-cols-2 gap-3 mb-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="relative overflow-hidden rounded-xl glass-card-animated" style={{ border: '1px solid rgba(139,92,246,0.75)' }}>
            ...
            <div className="relative z-10 p-4">
              <p className="text-white text-[11px] uppercase tracking-wider label-pill mb-1">Total Restaked</p>
          ...
          <div className="relative overflow-hidden rounded-xl glass-card-animated" style={{ border: '1px solid rgba(139,92,246,0.75)' }}>
            ...
            <div className="relative z-10 p-4">
              <p className="text-white text-[11px] uppercase tracking-wider label-pill mb-1">Bonus Yield</p>
              <p className="stat-value text-lg text-white">WETH</p>
              <p className="text-white text-[11px]">From protocol fees</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="relative overflow-hidden rounded-xl glass-card-animated" style={{ border: '1px solid rgba(139,92,246,0.75)' }}
          ...
          {!isConnected ? (
            <div className="text-center py-8">
              <p className="text-white text-[13px] mb-4">Connect wallet to restake</p>
            ...
            <div className="py-8 px-2">
              ...
              <p className="text-white text-[12px] leading-relaxed text-center max-w-md mx-auto mb-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 max-w-sm mx-auto">
                {[
                  ...
                  <div key={item.title} className="rounded-lg p-2.5 text-center" style={{ background: 'rgba(139,92,246,0.75)', border: '1px solid rgba(139,92,246,0.75)' }}>
                    <p className="text-white text-[11px] font-medium mb-0.5">{item.title}</p>
                    <p className="text-white text-[10px]">{item.desc}</p>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="rounded-lg p-3" style={{ background: 'rgba(139,92,246,0.75)', border: '1px solid rgba(139,92,246,0.75)' }}>
                  <p className="text-white text-[10px] mb-0.5">Restaked Amount</p>
                  <p className="stat-value text-[16px] text-white">{formatTokenAmount(formatEther(restakedAmount), 2)}</p>
                </div>
                <div className="rounded-lg p-3" style={{ background: 'rgba(139,92,246,0.75)', border: '1px solid rgba(139,92,246,0.75)' }}>
                  <p className="text-white text-[10px] mb-0.5">Pending Base (TOWELI)</p>
                  <p className="stat-value text-[16px] text-white">{formatTokenAmount(formatEther(pendingBase), 4)}</p>
                </div>
                <div className="rounded-lg p-3 col-span-2" style={{ background: 'rgba(45,139,78,0.04)', border: '1px solid rgba(45,139,78,0.12)' }}>
                  <p className="text-white text-[10px] mb-0.5">Pending Bonus (WETH)</p>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <button onClick={handleClaimAll}
                  ...
                  className="btn-primary flex-1 py-3 min-h-[44px] text-[14px] disabled:opacity-70 disabled:cursor-not-allowed">
                  {isPending || isConfirming ? 'Processing...' : 'Claim All Rewards'}
                </button>
                <button onClick={handleUnrestake}
                  disabled={isPending || isConfirming || isWrongNetwork}
                  className="btn-secondary flex-1 py-3 min-h-[44px] text-[14px] disabled:opacity-70">
            ...
            /* No staking position */
            <div className="text-center py-8">
              <p className="text-white text-[13px] mb-3">You need a staking position first</p>
            ...
            <div className="text-center py-6">
              ...
              <p className="text-white text-[12px] mb-4 max-w-md mx-auto">
                Deposit your tsTOWELI NFT (Position #{pos.tokenId.toString()}) to earn bonus WETH on top of your base TOWELI rewards.
              </p>
              <div className="rounded-lg p-4 mb-4 mx-auto max-w-xs" style={{ background: 'rgba(139,92,246,0.75)', border: '1px solid rgba(139,92,246,0.75)' }}>
                <p className="text-white text-[10px] mb-0.5">Your Position</p>
                <p className="stat-value text-[18px] text-white">{formatTokenAmount(pos.stakedFormatted, 2)} TOWELI</p>
                <p className="text-white text-[11px]">{pos.boostMultiplier.toFixed(2)}x boost</p>
              </div>
              {/* Step indicator */}
              <div className="flex items-center justify-center gap-3 mb-4">
                <div className={`flex items-center gap-1.5 ${isNFTApproved ? 'opacity-40' : ''}`}>
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px]" style={{ background: isNFTApproved ? 'rgba(34,197,94,0.3)' : 'rgba(139,92,246,0.2)', color: '#000000' }}>
                    {isNFTApproved ? '\u2713' : '1'}
                  </span>
                  <span className="text-white text-[11px]">Approve</span>
                </div>
                <span className="text-white text-[10px]">&rarr;</span>
                <div className={`flex items-center gap-1.5 ${!isNFTApproved ? 'opacity-40' : ''}`}>
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px]" style={{ background: 'rgba(139,92,246,0.2)', color: '#000000' }}>2</span>
              ...
              {!isNFTApproved ? (
                <button onClick={handleApprove}
                  disabled={isPending || isConfirming || isWrongNetwork}
                  className="btn-secondary px-8 py-3 min-h-[44px] text-[14px] disabled:opacity-70 disabled:cursor-not-allowed">
                  {isPending || isConfirming ? 'Approving...' : 'Step 1: Approve NFT'}
                </button>
              ) : (
                <button onClick={handleDeposit}
                  disabled={isPending || isConfirming || isWrongNetwork}
                  className="btn-primary px-8 py-3 min-h-[44px] text-[14px] disabled:opacity-70 disabled:cursor-not-allowed">
