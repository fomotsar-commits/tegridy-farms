...
const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      '37 invariant tests had never run either — AMM k-growth and LP-supply conservation, NFT-pool price monotonicity and fee accrual, staking and reward invariants, fee-router conservation, TWAP first-observation. They had been assumed too slow to gate; measured and tuned, they take 2.4 seconds while still driving 2,048 calls per invariant. All 37 passed on their first-ever execution',
      'The browser end-to-end suite had been allowed to fail without failing the build — an escape hatch added in May 2026 and never revisited. All six failing specs turned out to be stale test selectors rather than product bugs. Fixed, and the hatch removed. Now the part that matters, because leaving it out would be the same defect this entry is about: 44 of those tests — the ones that stake, swap, add and remove liquidity, borrow, repay and claim against a forked chain — are still skipped, because no pipeline supplies the chain they need. The machinery to run them was built the same week and deliberately not wired in yet. The green check covers the interface, not the money paths',
  ...
  },
  {
    ...
    items: [
      ...
      'The fee rules were published up front rather than left as a marketing dial. Of the 1% trade fee: 70% to the creator, 10% to attention beneficiaries the creator names at launch, 15% to the Tegridy treasury, 5% to Doppler — fixed at launch, not something turned afterwards',
