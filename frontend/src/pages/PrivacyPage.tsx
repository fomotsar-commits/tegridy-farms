...
const SECTIONS = [
  ...
  },
  {
    ...
    body: 'Client side (your browser): all user preference data is stored in your browser\'s localStorage and sessionStorage. You control it and can clear it at any time through your browser settings. Server side (Supabase): to make opt-in features work, we store the minimum data required, keyed only by public wallet address. The tables are: `messages` (chat posts you publish), `user_profiles` / `user_favorites` / `user_watchlist` (discovery surfaces tied to your wallet), `votes` (governance and opinion votes), `trade_offers` (peer-to-peer trade records), `push_subscriptions` (browser push endpoints you opt into), `native_orders` (signed marketplace orders), `siwe_nonces` (single-use auth nonces, auto-deleted within 5 minutes), `revoked_jwts` (per-session revocation list so logged-out tokens stop working — pruned as each token expires), and `error_events` (sanitized client-side error records — message, stack, page URL, timestamp; no wallet address and no session identifier, so rows cannot be linked to each other or to you). Row-Level Security is enforced on every row by the wallet claim in your SIWE-issued JWT; writes that don\'t match your wallet are rejected at the database. Blockchain (Ethereum): transaction data is permanently and publicly recorded by Ethereum itself. It is not controlled by the Protocol and cannot be altered or removed by any party.',
  },
  {
    id: 'your-rights',
    title: '6. Your Rights',
