...
| 4.2 | ✅ **shipped 2026-08-18 `21835d1d`** — `liveConfig.ts` `CONFIG_OFFSETS.feeClaimer` **72 → 40**; both offsets now pinned separately against a synthetic account whose two fields differ, so they cannot silently swap again | ~~⛔ Must precede 4.6.~~ Done — no longer gates 4.6. On v1 both offsets hold the same key so nothing is exposed — but the moment a v2 config is minted this is the only thing between a mistyped `fee_claimer` and every future launch's fees, **and it would report "verified"**. |
| 4.3 | ✅ **shipped 2026-08-18 `21835d1d`** — `--opening-fee-bps` / `--resting-fee-bps` / `--decay-seconds` on `cmdCreateConfig`; the resolved schedule prints in the dry run and anything above 5000 bps refuses without an explicit acknowledgement flag | ~~⛔ Must precede 4.6.~~ Done — no longer gates 4.6. Without it a signing session silently reproduces `DEFAULT_ANTI_SNIPE` = 9900 bps. |
...
| 4.6 | **DBC config v2** — print without `--send`, read the fee split, then sign | ✅ **both code gates (4.2, 4.3) are now shipped, so this is UNBLOCKED and purely operator.** **Independent of 4.1** — it is Meteora's program, not ours. Cheapest revenue-relevant action anywhere on this page. |
