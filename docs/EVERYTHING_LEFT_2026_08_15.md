...
| M.11 | ~~Custody decision for the reserve~~ **DECIDED 2026-08-22 (owner): recipient = the operator address `0x14898258122C0740106391E6e8E4F17F3b6d456E`, share = 3.69% (down from 5%).** Set in `config.ts` (ECOSYSTEM_RESERVE_BPS/RECIPIENT) and the curve deploy default. | ⚠️ This is the deployer **EOA** (0x code on-chain), not a Safe/vault — a deliberate interim (one key holds the accrued reserve; repointable to a Safe/vault later via `setLaunchConfig`, future launches only). The **CURVE rail is LIVE** with it (native carve→graduation transfer, event-logged, no Fact Sheet dependency): deploy with RESERVE_BPS=369 + this recipient. |
| M.12 | **Doppler-rail reserve activation** — the one remaining reserve code item, and it is genuinely operator-gated: it needs a DEDICATED Fact Sheet disclosure column, and the Fact Sheet is an **on-chain EAS schema** (`FACT_SHEET_EAS_SCHEMA`), so adding the column = re-register the schema (new UID = an operator on-chain write) + wire supply math + populate `ecosystemReserve`. | Deliberately still dark: the constants are inert on the Doppler path (nothing consumes them yet), so nothing undisclosed ships. Wiring the reserve WITHOUT the disclosure column would hide a 3.69% insider allocation — worse than dark. NEVER fold it into `teamAllocationVestedBps` (creator-only; disclosure tests pin it). The curve rail needs none of this. |
...
| # | Operator step | Notes |
|---|---|---|
| M.16 | **Own-curve go-live per chain**: `forge script script/curve/DeployCurveLauncher.s.sol` with FACTORY + MULTISIG + PAUSE_GUARDIAN + TREASURY + RESERVE_RECIPIENT | Owner = multisig AT BIRTH (no rotation step). **All economics now DEFAULT** (owner-decided 2026-08-23, `docs/CURVE_ECONOMICS.md`): fee 1%, split **40% creator / 25% treasury / 35% protocol**, reserve **3.69%**, graduation **mainnet 4 / Base 2 / Robinhood 1.5 ETH** (per-chain, `virtualEth = grad/19`). RESERVE_RECIPIENT + TREASURY = the operator address `0x14898258122C0740106391E6e8E4F17F3b6d456E` (same on every chain, EOA accepted with warning). Override any via env. Needs the chain's TegridyFactory (M.2) first; **mainnet curve is the go-live-first rail** (reuses the existing factory + multisig). |
