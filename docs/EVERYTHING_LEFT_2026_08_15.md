...
| 5.3 | **Send the Whetstone petition** (+ the BUSL grant question) | I refresh three stale claims first — including the branch pointer: `claude/launcher-own-venue` tip `3285bf02` does **not** contain `PROTOCOL_OWNER_MIN_SHARES`, so we would point them at a branch missing the exact fix we claim. `Airlock.owner()` re-checked and unchanged. |
