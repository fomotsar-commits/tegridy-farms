...
### Where this contract is implemented — ONE place

`frontend/src/lib/launcher/solana/curve/`, and nowhere else.

| file | what it owns |
| --- | --- |
| `math.ts` | the port of `curve.rs`. Imports nothing. **The only quote arithmetic in the repo.** |
| `curveVectors.fixture.ts` | 3,815 input/output rows generated by compiling and running the real `curve.rs` |
| `program.ts` | identity, PDA seeds, discriminators, the error table, account decoders |
| `ix.ts` | hand-encoded instruction builders, user and operator |
| `read.ts` | RPC reads, the phase classifier, the honest-unknown types |
| `rpc.ts` | the browser transport through `/api/solrpc`, the mint read, the write seam |
| `geometry.ts` | plot coordinates. Presentation; its one quoted number comes from `math.ts` |
| `config.ts` | operator pre-flight for `initialize_global` / `update_global` |
| `format.ts` | numbers and phases into words, with no default-to-zero anywhere |

This list exists because the surface was first built as four independent slices — a
page, a chart, an operator CLI and a client — and each wrote its own transcription of
`curve.rs`. Four sources of truth for quote maths is a UI that quotes differently than
the program executes, which takes money from users on every trade. The three duplicates
(`curve.ts`, `curvePoints.ts`, `tegridyLaunch.ts`) are deleted.

It was not a theoretical risk. Consolidating found a real divergence: the duplicates
disagreed about whether `graduation_price_ratio_bps` and `continuity_target` emulate
Rust's `u128` ceiling. The operator slice did and was right; the client core did not and
returned a confident number where the program returns `Overflow`. The core is fixed and
the fixture now samples the full `u64` range for both. **A new copy of this arithmetic is
a defect, whatever it is for.**
