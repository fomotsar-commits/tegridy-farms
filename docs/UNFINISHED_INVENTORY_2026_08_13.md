...
- **origin/main is dead as code but is still a production deploy path** `[S]` **[OPERATOR]** — **PARTLY RESOLVED 2026-09-10; see the correction below. It is no longer a deploy path, and it was not quite empty.**
  ...
  - **CORRECTED 2026-09-10 (T27) — two claims above are now wrong.** (1) *"is still a production deploy path"*: the repoint happened. `docs/TODO_OPERATOR.md` §3, landed 2026-09-06 in #435, records `mvp-launch` as the Production Branch, so step one of the ordering above is DONE and only "delete or lock main" is left — an owner's call, not an agent's. (2) *"carries zero unlanded work"*: not quite. A file-level diff (`git diff --name-status origin/main origin/mvp-launch`) finds ten paths on main and not on trunk; seven of those existed at the merge-base and trunk deliberately deleted them, and of the three main-only additions one is superseded (`frontend/api/aggregator/[provider]/[...path].js` → trunk's consolidated `frontend/api/aggregator.js`) — but **two test files were genuinely stranded**: `contracts/test/M19_RestoredContracts_AcceptOwnership.t.sol` (ported to trunk 2026-09-10; the guard it pins was on trunk with no coverage) and `contracts/test/StakingMonitorViewConstants.t.sol` (obsolete — it pins constants re-exposed on `StakingMonitorView`, and trunk's `StakingMonitorView` has none of them; `TegridyStaking` kept them `public`, so there is no second copy to drift). The four spot-checks cited below were commit-level and did not reach either file.
