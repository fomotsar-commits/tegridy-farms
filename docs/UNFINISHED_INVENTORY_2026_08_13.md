...
- **The exact 36-selector diff that brings TegridyRestaking under EIP-170 is recoverable right now from an orphaned artifact — I recovered it** `[M]`
  - `contracts/src/TegridyRestaking.sol`
  - The recorded estimate was "~1 h mechanical reconstruction" of a lost diff. It is now a checklist of 36 named items with a proven landing point (24,743 B) and a proven no-collision check against the view sister. The artifact lives in a gitignored agent worktree that any prune deletes — capture the list before it is gone.
  - *evidence:* contracts/src/TegridyRestaking.sol is unchanged and still over EIP-170 (local artifact 26,784 B; wt3 artifact 26,760 B; contracts-ci.yml:127 carries it as the sole OVER_EIP170_DEFERRED entry). None of the 2026-08-08 selector plan landed: claimPendingUnsettled() still at :1349, proposeSweepStuckRewards at :1753, _safeBonusTransferExt self-call trampoline at :2592. The plan documents (scratchpad/res
...
- **The Phase-7 restaking deploy script named in DeployMVP.s.sol was never written, and RestakingMonitorView has no deploy path at all** `[M]`
  - `contracts/script/DeployMVP.s.sol`
  - Restaking go-live needs five coordinated steps (host + RestakingAdminLib link + RestakingMonitorView + revDist/staking/referral wiring + guardian) and there is no script, no runbook section, and no VerifyRestaking invariant set. The gap is invisible because VerifyMVP.s.sol:48 explicitly excludes restaking from its checks.
  - *evidence:* contracts/script/DeployMVP.s.sol:209-210 says "Deploy + wire it in a dedicated Phase-7 script once TegridyRestaking is itself split under EIP-170." `grep -rl "new TegridyRestaking(" contracts/script/` returns nothing; `grep -rl "new RestakingMonitorView(" contracts/script/` returns nothing (only 5 test files construct it). RestakingMonitorView.sol is the only src contract that is neither deployed,
...
- **docs/RESTAKING_EIP170_SPLIT_DESIGN.md prescribes the approach that was disproven** `[S]`
  - `docs/RESTAKING_EIP170_SPLIT_DESIGN.md`
  - Anyone picking up "finish restaking" reads this doc, builds a sister contract, and lands roughly nowhere — a day-plus of the wrong work, on a fund-touching contract that then needs re-audit. Supersede it with the 36-selector list above.
  - *evidence:* The doc is the only in-repo plan of record for the restaking split. It opens "Status: DESIGN ONLY — not code" and §2.1 prescribes creating contracts/src/TegridyRestakingAdmin.sol and moving the timelocked admin surface off the host, estimating "the residual gap is on the order of ~2–4 KB". That is logic extraction — the lever the 2026-08-08 byte-attribution pass showed is exhausted (only 38.6% of 
