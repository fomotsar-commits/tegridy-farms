...
**Nothing has landed yet.** This file opens the wave, as §7 requires. Every element row is
NOT-STARTED or BLOCKED, and the evidence column stays empty until a sha exists. It is
amended at every phase close, not written once.
...
| B the instrument is the hero | NOT-STARTED | | The wave's engine; everything else clears ground for it. Unblocked upstream — `x_handle` is served. `TIER_FLOORS` confirmed at `heatOracle.ts:27-33`; `heatLaunchFloor()` confirmed. `parseHeatReading` whitelists its output fields, so `x_handle` is dropped today and must be added to `HeatReading` without letting a missing handle read as an envelope failure. |
...
| G the board on the venue, and the flames proxy | NOT-STARTED | | Upstream **confirmed live** (see probe table). Placement law verified at `frontend/api/aggregator.js:303` — the branch must sit above `const provider`. Conflict with N on the cache header, below. Carries A's "Watch the arrival" film mount and the missing `id="hall"`. |
