...
**vitest 5552/5552 across 380 files** over `src/components`, `src/pages` and `src/lib`, on a
quiet machine (`--maxWorkers=2 --testTimeout=30000`, 333 s). Typecheck 0 across both projects
(`tsc -b --force` and `tsc -p tsconfig.test.json`). eslint 0 errors on every file this wave
has touched. Server-side: 774/774 across the heat and aggregator suites.
