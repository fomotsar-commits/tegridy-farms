...
| E zero unasked overlays | IN-PROGRESS | `2056b313` · `fc0e51c7` |✅ Two of three sites landed: the MuseBubble mount is gone from every room and the component **deleted rather than exempted** (the reachability guard's own words), and the room's welcome is INVITED behind "About this bungalow" via `OPEN_BUNGALOW_ABOUT_EVENT`, one mechanism shared with the venue's welcome. ⏳ **The third site is untouched:** `pwa/InstallPrompt.tsx:92` is still a fixed `inset-x-3 bottom-20 z-[9500]` banner mounted by `PwaRuntime.tsx:32`, opening itself on `beforeinstallprompt`. It becomes a footer row ("Add to home screen") that calls the deferred prompt on tap. DONE needs that and the §2 e2e. **Fair hit from the island: this row read NOT-STARTED while the code was two commits ahead** — a row that lags the code is the same defect as code claimed without a row. Anchors confirmed. Deleting the `MuseBubble` mount **reds two currently-green e2e outright**, not by flake: `bungalow-doors.spec.ts:116` expects the `— the island` byline on /pepe (it comes only from MuseBubble), and `ambient-bubble-taps.spec.ts:96` asserts the bubble rendered. Both must land in the same commit as the deletion.|
