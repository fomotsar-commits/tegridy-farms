...
4. **When you finish an item, update this file** under its heading, and note anything the instructions got wrong. This document is a living runbook — a wrong instruction left uncorrected is a trap for the next session. There are exactly two status markers, and the difference between them is the whole point:
   - `✅ shipped <date> <commit>` — **every clause of the item's Done-when is satisfied in production.** Done-when clauses in this plan are deliberately written as observable outcomes (a fee event on-chain, an indexer row, a claim that succeeded), so this marker is a claim about the world, not about the diff.
   - `🟡 in the tree <date> <commit> — <what is still missing>` — the code is merged and tested and the Done-when is not met. Nearly everything this repo builds lands here first, because contracts ship deployed nowhere and features ship flag-gated off by house law. The trailing clause is mandatory: an item marked without naming its gap is worse than an unmarked one, because it reads as finished.

   Merging is not shipping. A green suite proves the code does what its author believed; it cannot prove a contract was deployed, a dial was set, or a host exists. If you catch yourself writing `✅` on the strength of a passing test, the marker you want is `🟡`.
5. **When an instruction conflicts with repo reality, reality wins** — then fix the instruction. The repo moves; this plan was drafted 2026-08-18 and reconciled against the tree on 2026-08-19.
...
> 🟡 **in the tree 2026-08-18 `088ed89e`** — step 3 of the build is done and steps 1, 4 and 5 are not. Landed: `frontend/src/lib/indexer/client.ts` (fetch + zod, `VITE_INDEXER_URL`-gated), `queries.ts`, `hooks/useIndexedQuery.ts`, `useIndexedSwaps.ts`, `useIndexedStakingHistory.ts`, and `indexer/DEPLOY.md` + `.env.local.example` documenting the Railway/proxy steps. **Missing: the host.** There is no deployed endpoint, so `VITE_INDEXER_URL` is unset in every environment and the consumer hooks are mounted on no page — they are carried as an explicit exemption in `frontend/src/hooks/hooksAreMounted.test.ts` with that reason, precisely so an unhosted indexer cannot masquerade as a wired one. No Solana leg, no new-pair NOTIFY. Every Done-when clause below is unmet.
...
> 🟡 **in the tree 2026-08-18 `2840ce4f`** — step 2's first leg only, shipped switched off. `frontend/src/lib/fees/swapFee.ts` is the single policy: rate and recipient are read at call time from `VITE_SWAP_FEE_BPS` / `VITE_SWAP_FEE_RECIPIENT`, both unset by default, and the fee is disabled unless **both** parse — there is deliberately no fallback recipient, because a fee with nowhere to go must not pick a destination. `AggregatorQuote.venueFeeBps` records what was actually attached per provider, and `components/swap/VenueFeeLine.tsx` displays that number rather than the policy's, so the shown fee and the sent fee cannot diverge. **Only the ParaSwap leg is fed**; the other six providers are withheld on purpose because their parameter *units* could not be confirmed from this repo, and a correct parameter name carrying a percent where bps was meant is a 100× overcharge no surface would catch. Steps 1, 3 and 4 — the operator's POL/feeSplit timelock sequence, any periphery FeeRouter, the referral ledger — are untouched. No fee has been charged.
...
> 🟡 **partially in the tree** — step 5 is done: the coverage ratchet is armed (`.github/coverage-floor.json`, 2026-08-18 `abdbf38a`), and the same commit added the npm advisory gate the year plan asked for. Step 1's discipline is being followed by hand rather than by machine — `contracts/src/vendor/uniswap-merkle-distributor/VENDOR.md` (2026-08-18 `fd706689`) records the upstream repo, the pinned commit, the license, and one declared delta (a pragma widened from `=0.8.17` to `^0.8.17` so it compiles under this repo's solc), with the `curl | diff` command to re-verify by hand. That is the content `FORK_INTAKE.md` prescribes, but the document itself does not exist yet and neither does `scripts/diff-guard.sh` or `contracts/fork-manifest.json`, so **nothing in CI notices if the vendored tree drifts from its pin.** No echidna/halmos jobs, no `contracts/test/fork/ForkBase.t.sol`, no audit-wave gate on `deploy-gated.sh`. The Safe ceremony remains blocked on the Q1 custody re-home.
...
> 🟡 **in the tree 2026-08-19 `477acb2b`** — the *disclosure* half only; no migrator contract exists. `frontend/src/lib/launcher/graduation/venue.ts` + `feeLine.ts` and `components/launcher/GraduationVenuePanel.tsx` (mounted on `LaunchPage`) tell a creator where a launch actually graduates and what the fee line at that venue really holds, reading it rather than asserting it. **Missing: everything the Done-when measures.** `contracts/src/TegridyV2GraduationMigrator.sol` is not written, no deploy script exists, the Whetstone module whitelisting has not been requested, and there is no `graduation` table in the indexer schema. Today a launch graduates to a third-party venue, and the new panel's job is to stop the app implying otherwise.
...
> 🟡 **in the tree 2026-08-18 `2840ce4f`** — the fee substrate, switched off; see F3 above for the mechanism. Against this item's own build list: the per-provider fee legs are **1 of 6 fed** (ParaSwap; the rest withheld until their parameter units are confirmed against the provider's own docs, not this repo's), the CoW `partnerFee` appData field is defined and hash-pinned against `src/lib/cowProtocol.ts`, and the pre-signature disclosure exists. **Not built:** `UltraModePanel.tsx`, the net-of-fee re-ranking in `routeSavings.ts`, and the `allowedQuery` widening in `api/aggregator.js`. **Correction to the instructions:** this item assumed aggregator routes execute through the aggregator. They do not — aggregator quotes are used for comparison and the swap executes through the venue's own router, so a fee attached today would ride a route nobody takes. Enabling this needs a collection path decided first, not just a rate and a recipient.
...
**Fee wiring:** 0.1% per fill via the item-3 CoW partner-fee appData (payer: the DCA user, disclosed on schedule creation) → treasury Safe → F3 sweep → RevenueDistributor split. Phase 2 adds a 10% skim of realized idle-yield only (never principal), sent by the vault straight to RevenueDistributor's open `receive()` — `stakerShareBps` currently earmarks that arrival for stakers, and unlike the swap-fee rail there is no referral carve in front of it.
...
> 🟡 **in the tree 2026-08-18 `fd706689` (contracts) + 2026-08-19 `b9ff0c17` (front end)** — **deployed nowhere.** `contracts/src/AirdropFactory.sol` + `TegridyAirdropDistributor.sol` over a verbatim vendored Uniswap `merkle-distributor` (pin and single declared delta in `contracts/src/vendor/uniswap-merkle-distributor/VENDOR.md`), with an invariant suite. Front end: `pages/AirdropPage.tsx` (routed), `components/airdrop/CampaignBuilder.tsx` + `ClaimPanel.tsx`, `lib/merkle/`. **Correction to the instructions:** the tree is built **client-side from a CSV**, not from indexer holder snapshots, and there is no `api/_lib/airdrop.js` — which drops the F1 precondition and the Heat-floor filtering, and also means no server ever sees the recipient list. The leaf encoding is derived from the Solidity and pinned by a round-trip vector test, because a subtly different encoding produces a tree that looks right and rejects every claimant. `AIRDROP_FACTORY_ADDRESS` is the zero address, so every surface renders its deploy gate; no campaign exists and no claim fee has been charged.
...
> 🟡 **in the tree 2026-08-19 `fe59fc63`** — the Safe/ComposableCoW path was built first, exactly as the Gates line prescribes, and it is honest about what it cannot do. `lib/triggers/triggerPlan.ts` holds the pure integer math (every displayed number and every signed number come from one place; rounding is chosen so error always costs the venue), `stopLossHandler.ts` encodes CoW's StopLoss `Data`, `triggerFee.ts` the fee, and `armState.ts` is the honesty core: `armed` is computed from facts that were checked, never inferred from an order having been built or saved, and every unarmed path carries the reason in words a trader can act on. `KEEPER_AVAILABLE` is a **constant, not an env dial** — an operator cannot make a keeper exist by setting a variable, and a dial would invite exactly that. **Missing:** F4 (so every EOA and every kind CoW's handler cannot express lands unarmed by design), F6 session keys, the Solana leg, the Supabase order table, and the fee route — no `TerminalFeeRouter` exists. **Also: `TriggerOrderTab.tsx` is imported by nothing outside its own test**, so the surface is unreachable from the app. `hooksAreMounted.test.ts` cannot see this, because the hook it guards *is* imported — by the unmounted component.
...
**The build:** Contracts: apply every SCHEDULED fix (**21**, not “~15” — the ledger carries 43 findings: 21 `**Disposition: SCHEDULED`, 22 `**Disposition: MOOT`) in docs/SOLANA_PROGRAM_FINDINGS_2026_08_15.md to solana/tegridy-amm/programs/tegridy-launch and the vendored cp-swap fork; generate fresh keypairs, update declare_id! in both programs and Anchor.toml; rehearse on devnet with deploy-devnet.sh, then solana/tegridy-amm/MAINNET_RUNBOOK.md. Client: the three proven client findings **already landed on trunk** in `21835d1d` (2026-08-18) — do not re-apply them, and in particular **do NOT set `BONDING_CURVE_SIZE = 716`**. The ledger's 716 was the pre-removal layout; post-removal the curve is **170 bytes**, and 716 now rejects every real curve because the decoder gates on strict size equality (`frontend/src/lib/launcher/solana/curve/program.ts:623` returns `bad-length` unless `data.length === BONDING_CURVE_SIZE`). 170 is derived, not quoted: the account is allocated `space = 8 + BondingCurve::INIT_SPACE` (`solana/tegridy-amm/programs/tegridy-launch/src/lib.rs:1655`, and again at `:1949`), and `BondingCurve` (`.../src/state.rs:173-219`, `#[account] #[derive(InitSpace)]`) is `mint: Pubkey` 32 + `creator: Pubkey` 32 + eight `u64` 64 + `complete: bool` 1 + `pool: Pubkey` 32 + `bump: u8` 1 = **162**, so 8 + 162 = **170**. The shipped client already carries it (`program.ts:455-489`, `BONDING_CURVE_LAYOUT.size = 170`), the `creator` account is already in `tradeKeys` (`curve/ix.ts:216`), and the rent floors are already read from the cluster rather than hardcoded (`curve/read.ts:292-309`, `readRentFloors`). What is still owed is the last clause: assert the decoder against a captured base64 curve blob, not a locally-encoded struct — `getProgramAccounts` on the launch program still returns empty, so there are no real bytes to pin against until the redeploy creates one. Deploy order per findings: deploy → initialize_global → update_global with cp_swap_program/amm_config → prove one dust launch graduating end-to-end via migrate_to_amm → ONLY THEN rotate global.authority to the proven Squads vault. Frontend: point `frontend/src/lib/launcher/solana/curve/program.ts:48` (`PROGRAM_ID`, currently the spent `CpFnacrA…`) and `:60` (`CP_SWAP_PROGRAM_ID`, currently the closed `3ZvZXEBr…`) at the new ids — `solana/liveConfig.ts` no longer exists, it was deleted with the Meteora DBC rail in `ba871e0e`; un-gate CurveLaunchPage.tsx; enforce Heat floor 80 via src/lib/heat/launchGate.ts before create_launch. Data: F1 tables for curve trades, graduations, fee sweeps.
...
> 🟡 **in the tree 2026-08-19 `fe59fc63`, default 0** — and built in a shape this item did not anticipate, which is worth reading before touching it. `frontend/src/lib/launcher/launchPricing.ts` moves the boundary between the venue's own line of a launch's fee constitution and the creator's, at launch-creation time. It needs **no contract**: a launch's constitution sums to exactly 10000 bps and is fixed on-chain at creation by the locker's beneficiary set, Doppler's floor is not ours to move, and the total is an identity — so `creatorBonusBps === STANDARD_VENUE_LINE_BPS - venueBps` holds by construction. Two invariants are enforced and tested: enabling the feature can only ever make a launch *cheaper* than it is today, and a colder Heat tier can never be cheaper than a warmer one. The Heat tier is **read** from the existing gate and never recomputed. The default share is 0 and an out-of-band override is ignored rather than obeyed. **Not built:** `CreatorShareSplitter.sol`, so there is no ongoing claimable accrual — this is a share of the fee constitution set once at launch, not a claim rail. No `CreatorEarningsPanel.tsx`, no `?resource=creator-earnings`, no FDV tiering (that part does still need F1), and `components/launcher/FactSheetPricing.tsx` is written but imported by nothing, so the disclosure it renders reaches no page yet. Every Done-when clause is unmet.
...
> 🟡 **in the tree 2026-08-19 `206b4dc6`** — **deployed nowhere.** `contracts/src/LaunchRugEscrow.sol` + `contracts/script/DeployLaunchRugEscrow.s.sol`, immutable, no proxy, venue fee dial at zero, with an adversarial suite covering trigger-griefing and reentrancy. **Correction to the instructions, and it is the important one: the trigger is NOT an F8 signed attestation.** The plan specified a venue-signed attestation that registered wallets sold, posted on-chain behind a challenge window. That was rejected on build: a judgement-based trigger makes the contract a discretionary seizure tool wearing insurance clothes — it invites the venue to be lobbied and lets a creator be robbed by whoever holds the pen. What shipped instead is a covenant published at `open` and immutable thereafter, of the form "these named addresses will together hold at least `minBps` of this asset's supply until `windowEnd`", checked against present state; token-dump and LP-pull are the same check, so there is one code path to audit. Every ambiguity resolves for the creator (the held-share denominator is `min(liveSupply, snapshotAtOpen)`, so a third party burning tokens is not a breach, and dilution by minting cannot manufacture one). **This drops F8 and F1 from the preconditions.** Still missing: no deploy, no `RugEscrowPanel.tsx`, no `?resource=escrow-status`, no fact-sheet terms, no indexer tables — and no front end at all, so nothing user-facing offers this yet.
...
> 🟡 **in the tree 2026-08-19 `206b4dc6`** — **deployed nowhere.** `contracts/src/v4/DecayingFeeHook.sol` over OpenZeppelin's `uniswap-hooks` `BaseOverrideFee` inherited unmodified (only the schedule is ours), plus `contracts/script/DeployDecayingFeeHook.s.sol`. The pool-fee-not-token-tax constraint this item was written around is held and documented in the source, so `gate.ts`'s Tier-L disqualifier stays intact. A schedule is written once by `configurePool` **before** the pool exists and is immutable after — a pool cannot be initialised without a published schedule, because a decaying fee nobody can read in advance is just a surprise tax. **Correction to the instructions:** the fee-wiring line above prescribes a 50/25/25 split of "collected decay-fee proceeds" through `TegridyFeeLocker`. The hook collects nothing: it returns the elevated fee to the PoolManager through the standard override flag, and v4 credits it to in-range liquidity. There is no skim, no custody and no transfer in the contract, so where the proceeds land is a question about who holds the liquidity — at t≈0, the launch position itself — not a split this hook can perform. Rewrite that line before anyone builds a splitter for it. **Note:** commit `206b4dc6` labels this item "#13"; #13 is the graduation priority auction and is not built. Still missing: deploy, the wizard toggle, the live effective-fee display, the `CurveChart` overlay, and the `decay_fee_collection` table.
...
> 🟡 **in the tree 2026-08-18 `fd706689` (contracts) + 2026-08-19 `b9ff0c17` (front end)** — **deployed nowhere.** `VestingFactory.sol` over OZ `VestingWallet`, `TegridyLockVault.sol` (time-locked ERC-20/LP custody whose owner has no withdraw, rescue, sweep, shorten or reassign path, whose `withdraw` is deliberately not `whenNotPaused` so a pause cannot trap a matured lock, and whose `extend` is monotonic so "locked until T" can never be made retroactively false), and `LaunchLockView.sol` joining both into one call for `gate.ts`. Front end: `pages/VestingPage.tsx` (routed), `components/vesting/`, `hooks/useVestingFactory.ts` / `useVestingStreams.ts` / `useVestingLockView.ts`. **Correction to the instructions:** no Sablier integration exists. The plan said integrate the canonical deployed Sablier Lockup and vendor OZ `VestingWallet` only for the simple leg; what shipped is the OZ leg plus a house lock vault, and `collector.ts`'s injectable `LockResolver` is still unfilled — so `gate.ts`'s Tier-L lock checks still fall back to their conservative defaults rather than reading live data, which is Done-when clause 2 and it is unmet. `LockViewer` renders unset rails as NO DATA rather than as a zero balance, which is the whole reason the availability flags exist. Also missing: the wizard step, `?resource=locks`, the indexer tables, and the 0.25% fee leg.
...
- **Copy law:** all new surfaces must match the post-honesty-pass fee claim — "stakers + liquidity engine + operations". Totality claims about the swap fee reaching stakers are banned outright: a non-zero, non-removable referral share sits in front of the distributor, so no share above ~80% can ever arrive. Describe the route (routed, earmarked), never assert arrival. `revenueClaimHonesty` / `deployClaimHonesty` / `docsClaimHonesty` fail dishonest copy — and `docsClaimHonesty` does not exempt quoted corrections, so describe a banned phrase rather than reproducing it.
