...
> Status: **not yet connected.** No Dune integration exists in the repo — this doc is the
> starter kit. Create the dashboard on dune.com, paste these, then embed (see bottom).
> Best run once real volume exists (post-LP-seed).
...
WITH swaps AS (
  SELECT
    block_time,
    bytearray_to_uint256(bytearray_substring(data, 97, 32)) / 1e18 AS fee_eth,
...
SELECT
  ...
  sum(bytearray_to_uint256(bytearray_substring(data, 1, 32)) / 1e18) AS eth_distributed_day,
  sum(sum(bytearray_to_uint256(bytearray_substring(data, 1, 32)) / 1e18))
...
SELECT
  ...
  sum(bytearray_to_uint256(bytearray_substring(data, 1, 32)) / 1e18)  AS eth_claimed_total
FROM ethereum.logs
WHERE contract_address = 0xf993316e2fc079de4358c489a935e01e03e23e17
...
SELECT
  block_time,
  bytearray_to_uint256(bytearray_substring(data, 1, 32)) / 1e18 AS toweli_funded
FROM ethereum.logs
...
ORDER BY block_time;
-- (Current: one 6,400,000 TOWELI funding on 2026-06-06.)
```
...
SELECT
  ...
  sum((bytearray_to_uint256(bytearray_substring(data, 33, 32))      -- amount1In  (WETH in)
     + bytearray_to_uint256(bytearray_substring(data, 97, 32))) / 1e18) AS weth_volume_day -- amount1Out (WETH out)
FROM ethereum.logs
