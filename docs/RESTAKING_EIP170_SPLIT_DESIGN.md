...
Status: **EXECUTED — this is code, landed 2026-08-19 in `c749c933`.** §1–4 describe what was built; §5 is a completed checklist. **One item is still open — §5.8, the mandatory external re-audit**, which gates any deploy because the moved surface is fund-touching.

Measured deployed runtime bytecode after the split (`address(c).code.length` — the quantity EIP-170 bounds at CREATE, pinned as a regression guard in `contracts/test/Audit_RestakingEIP170Size.t.sol`):

| Contract | Deployed runtime | vs. EIP-170 (24,576 B) |
|---|---|---|
| `TegridyRestaking` (host) | **22,114 B** — was 26,784 B, **−4,670 B** | **2,462 B under** |
| `TegridyRestakingAdmin` (admin sister, new) | 9,298 B | 15,278 B under |
| `RestakingMonitorView` (view sister, unchanged) | 2,275 B | 22,301 B under |

The pre-split host was **2,208 B over** the limit; moving the timelocked admin surface to the sister reclaimed **4,670 B — roughly twice the gap**. The figures above are the `forge build --sizes` runtime column, re-measured in a fresh worktree on 2026-08-19 (~10 min cold — `[[reference_local_forge_build_hang]]`); CI `Contracts CI` remains the gate of record. TegridyRestaking has **never been deployed** (deferred to Phase 7 per `contracts/script/DeployMVP.s.sol:202-213`), so this was a fresh-deploy split with **no storage-migration constraint**.

### As-built deviations from the design below

Two things landed differently from what this document originally prescribed. Both are deliberate. §2.1/§3.3/§3.4 have been corrected in place; they are also listed here because a reader who already knows this design would not otherwise think to look for them.

1. **Only the `*Proposed` and `*Cancelled` events moved to the sister.** The five `*Executed` events — `BonusRateExecuted`, `StuckBaseRewardsAttributed`, `SweepStuckExecuted`, `RescueNFTExecuted`, `ResidualClearExecuted` — stay on the **host**, emitted at the site that performs the write, so an execution log can never appear without the corresponding state change. §3.4 originally implied all three families relocate.
2. **The deprecated revert stubs were deleted outright, not carried over.** `setBonusRewardPerSecond(uint256)` and `sweepStuckRewards(address)` are gone from the host and were not re-created on the sister — neither name appears anywhere in `contracts/src/`. A caller of either now gets a plain no-such-function revert instead of the typed deprecation revert.

Ground-truth files read when this design was written (state *before* the split — line numbers below refer to the pre-split host, which was 2,629 lines and is now 2,475):
- `contracts/src/TegridyRestaking.sol` (2,629 lines — the over-limit host; now 2,475 lines and under the limit)
- `contracts/src/lib/RestakingAdminLib.sol` (part-1 delegatecall lib already extracting cold admin *bodies*) — **deleted by this split; its bodies now live in the host `applyXxx` hooks, see §4.5**
...
3. **Timelocked admin surface (the move target)** — every function, pending-state var, auto-getter, event and key in this list is owner-only param governance, none of it on any user hot path:
   ...
   Moving all of the above off the host (and with it the `RestakingAdminLib` import/link at `:25`, `:383`, `:395`, `:422`, `:1590`, `:1743`, `:1889`, `:2047`) plus dropping `TimelockAdmin` inheritance is the reliable path under 24,576 B. Order-of-magnitude reclaim: ~12 external selectors + their bodies, ~7 auto-getters, 5 `bytes32`/pending slots, the 44-line `acceptOwnership` flush, and the entire `TimelockAdmin` base — comfortably in the multi-KB range, i.e. more than the residual gap. **CI `--sizes` confirms; do not treat the estimate as sufficient.** **Outcome (measured):** the estimate held — this move alone reclaimed **4,670 B**, landing the host at **22,114 B**, 2,462 B under the limit, with the fund/accounting core untouched.
...
It holds **all** propose/execute/cancel triplets, all pending state, all timelock keys/delays, the anti-churn cooldown, the residual-clear 7-day mapping, and the `acceptOwnership` flush. It owns **no funds and no pausable state** (no `PauseGuardian`, no `ReentrancyGuard` needed — it never receives a reentrant callback; see §4.1). Governance **`*Proposed` / `*Cancelled`** events are **re-declared and emitted from the sister** (topic-identical, but now emitted at the *sister* address). **As built, the `*Executed` events stayed on the host** — see §3.4.
...
- Sweep: `CannotSweepBonusToken`/`CannotSweepRewardToken`/`ZeroAddress` at propose, re-checked at execute; full balance to `address(staking)`; `SweepStuckProposed`/`SweepStuckExecuted(token, amount)`/`SweepStuckCancelled`. `sweepStuckRewards` deprecated-revert stub **was deleted outright** (no on-chain consumer), as was `setBonusRewardPerSecond`.
...
**As built, the relocation is by event family, not wholesale.** The `*Proposed` and `*Cancelled` governance events now emit from the **sister address**. The five `*Executed` events **stayed on the host**, each emitted inside the `applyXxx` that performs the write — `BonusRateExecuted` (`TegridyRestaking.sol:1874`), `StuckBaseRewardsAttributed` (`:1897`), `SweepStuckExecuted` (`:1913`), `RescueNFTExecuted` (`:1930`), `ResidualClearExecuted` (`:1946`) — so a log-level reader can never see an execution that did not happen, which a sister-side emit could not guarantee. Topic hashes are identical in both families (same signatures). **Consequence:** any `vm.expectEmit` for a `*Proposed`/`*Cancelled` event, and any indexer filtering those by the host address, must repoint to the **sister**; `*Executed` expectations and filters stay bound to the **host** and need no change. **Fund events stay on the host and are unchanged**: `Restaked`, `Unrestaked`, `BonusClaimed`, `BaseClaimed`, `BonusFunded`, `EmergencyWithdraw`, `UnsettledRecovered`, `ResidualReserved`/`ResidualClaimed`/`ResidualPullDeferredCrossHolder`/`ResidualClaimWaived`, `RestakeNFTStranded`/`Reclaimed`, `BonusShortfall`, `PositionRefreshed`, `BoostRevalidated`, `BonusTransferDeferred`, `EmergencyForceReturn`.
...
- `RestakingAdminLib` became unused by the host and **`contracts/src/lib/RestakingAdminLib.sol` was deleted** rather than re-linked. Its `executeClearResidualClaimant` wrote `residualClaimant_` **directly**, which cannot survive the cross-contract boundary (the write must be `host.applyResidualClaimant`), so every moved body was re-expressed and **must be re-audited** — do not assume any of it is byte-identical to the previously-audited library.
...
## 5. Execution checklist — **COMPLETED 2026-08-19 (`c749c933`), except item 8**

1. ✅ **Branch + baseline size.** Baseline host runtime recorded at **26,784 B** — 2,208 B over EIP-170, undeployable by construction.
2. ✅ **Sister written** — `contracts/src/TegridyRestakingAdmin.sol` (`OwnableNoRenounce, TimelockAdmin`, `ITegridyRestakingApply`): all five propose/execute/cancel triplets, all pending state, the four timelock keys + delays, the 24h anti-churn cooldown, the 7-day residual-clear mapping, and the `acceptOwnership` flush (which also resets `lastBonusRateActionAt` per F-2). **9,298 B.**
3. ✅ **Host edited** — `TegridyRestaking.sol` no longer inherits `TimelockAdmin` (`:111`) and no longer references `RestakingAdminLib`; the §1.2 admin surface is deleted. Added `restakingAdmin` (`:1792`), `setRestakingAdmin` (`:1809`), inline `proposeAdminReplacement` / `executeAdminReplacement` / `cancelAdminReplacement` (`:1818` / `:1833` / `:1847`), `onlyAdmin` (`:1855`), and the five `applyXxx` hooks (`:1870`, `:1885`, `:1907`, `:1925`, `:1939`). Pause/guardian/`emergencyForceReturn`/user-fund paths untouched. `contracts/src/lib/RestakingAdminLib.sol` deleted.
4. ✅ **Pointer wiring reviewed** — all five `applyXxx` are `onlyAdmin`, the sister's triplets are `onlyOwner`, and the EOA/7702 rejection (`codeLen == 0 || codeLen == 23`) is on both `setRestakingAdmin` and `proposeAdminReplacement`. The two fund-touching hooks (`applySweepStuckRewards`, `applyRescueNFT`) additionally carry `nonReentrant` per §4.1.
5. ✅ **Deploy script** — `contracts/script/DeployRestaking.s.sol`: `new TegridyRestaking(...)` (`:67`) → `new TegridyRestakingAdmin(host)` (`:72`) → `host.setRestakingAdmin(sister)` (`:77`) → `setPauseGuardian` (`:86`) → `transferOwnership(multisig)` on **both** (`:92-93`), ownership last because `setRestakingAdmin` is `onlyOwner` and one-shot. The cross-wiring the MVP script defers (`stakingAdmin.proposeRestakingContract` / `revDist.proposeRestakingChange`, 48h each) is printed as an operator follow-up at `:105`.
6. ✅ **Sizes confirmed** — host **22,114 B**, admin sister **9,298 B**, view sister **2,275 B**; all three under 24,576 B, the host with **2,462 B** of headroom. No escalation to further view extraction was needed. Drift is now pinned by `contracts/test/Audit_RestakingEIP170Size.t.sol`, which asserts the host under both the 24,576 B limit and a conservative 24,000 B floor, and logs all three sizes so CI records headroom at the commit that changes it.
7. ✅ **Format + suite** — the §3.5 suites were repointed at the sister (**102** `restakingAdmin.` call sites — 121 total `restakingAdmin` references, of which 19 are declarations, constructor assignments and comments — across `TegridyRestaking.t.sol`, `Audit195_Restaking.t.sol`, `Deep_Restaking_2026_05_01.t.sol`, `FinalAudit_Restaking.t.sol`), with `*Executed` expectations left bound to the host per §3.4. The suite itself was **not** re-run locally — green/red is CI `Contracts CI`'s verdict (`[[reference_local_forge_build_hang]]`, `[[reference_slither_gate]]`; raw Slither is chronically red, ignore per memory).
8. ⬜ **OPEN — MANDATORY external re-audit before any deploy.** *(The one item of this checklist still outstanding.)* This surface is fund-touching (attribution credits, NFT rescue, residual-claimant writes, sweep-to-staking) and the split re-expresses previously-audited bodies across a new trust boundary. Per `[[project_2026_07_16_gated_batch_deployed.md]]` do not un-gate / deploy fund-touching features without a per-feature audit, and do not accept ownership into a flagged Safe — rebuild the Safe and re-home first.
