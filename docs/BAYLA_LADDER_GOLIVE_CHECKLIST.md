...
ladder, so the program changes, the mainnet artifact `fada8148…` is **SUPERSEDED**, and the
...
1. The code is **being rebuilt**: a 75% early-exit penalty and a reload rate guard (branch
   `feat/bayla-ladder-75-penalty`). The frontend's own copy of the penalty
   (`frontend/src/lib/ladder/program.ts:76`, `2_500` on trunk) ships at `7_500` **in
   lockstep with the mainnet deploy**; beyond that one constant, go-live is two environment
   variables and a redeploy (§6).
2. The program is **not deployed**. The address is unclaimed, the deployer is being
   rotated, and the artifact built so far (`fada8148…`) is superseded.
...
| D5 | **Matured positions keep full boost indefinitely — disclosed, not changed.** Weight is frozen at stake time (`state.rs:132-133`) with no forced maturity, decay or kick (`state.rs:51-52`). A future upgrade MAY reset matured weight to 0.40x. | disclosure |
...
- **The 09-12 mainnet artifact is SUPERSEDED.** `.so` sha256
  ...
  carries the 25% penalty, no rate guard, and the hot faucet key `GCCSLE7d…` as its
...
- **The external audit must cover the constant change and the rate guard.** A sign-off on
  ...
  signed off on" (§2).
- **The frontend penalty constant ships in lockstep with the mainnet deploy.**
  `program.ts:76` is a separate literal from the program's `math.rs` constant, and nothing
  on chain exposes the rate, so a card built at one rate against a program at the other
  misquotes every early exit (§6).
...
| Pool authority (compiled in) | `GCCSLE7dBPMijj5F4pDxe592mcGAK83N84R2w5HPauV9` — **in the superseded `fada8148…` build only.** Rotated under option (A), D3: the rebuild compiles a fresh key, not yet recorded | `BAYLA_LADDER_MAINNET_RUNBOOK.md` §1 |
...
- **(A) Rotate before deploying — recommended, and CHOSEN.** Generate a fresh deployer on a
  ...
  `deployer`, and use it. Cost: one CI run, and the current `fada8148…` artifact is void —
  which the 75% rebuild requires anyway.
- **(B) Deploy with `GCCSLE7d…`, rotate the authority immediately after init — NOT
...
(6028). A sign-off on code from before them — or on the superseded `fada8148…` build —
...
- [ ] ~~**Download the mainnet artifact and verify its hash.**~~ **SUPERSEDED 2026-09-17 —
      do not download or deploy `fada8148…`** (25% penalty, no rate guard, hot faucet
      ...
      (`3e1b2b7b…`), and the binaries have been the same size — verify by **hash**, never by
      filename or size.
...
`lib.rs:861` and `:894` on trunk — and `math.rs:72-75` is only the penalty constant.)
...
- **The early-exit penalty is a compile-time constant with no setter: 75% forfeited, 25%
  returned** (`EARLY_EXIT_PENALTY_BPS = 7_500` in the rebuild's `math.rs`; `2_500` at
  `math.rs:75` on trunk until it lands). Only a program upgrade through the Squads vault
  could change it (D6).
...
| 4 | Rebuild the artifact from the audited commit | — | runbook §4, `deployer` = the rotated key. **Never `fada8148…`** |
| 5 | Fund the deployer ~3 SOL | — | ~2.62 needed; the **rotated** deployer — never `GCCSLE7d…` or `5MtoeJ8D…` |
...
| 8 | Verify deployed bytes | — | `program dump` + hash vs the **rebuilt** artifact's sha256 (never `fada8148…`) |
...
**One code change is required, and it ships in lockstep with the mainnet deploy:** the
card's penalty constant, `frontend/src/lib/ladder/program.ts:76`
(`EARLY_EXIT_PENALTY_BPS = 2_500` on trunk), becomes `7_500`. It is a separate literal from
the program's `math.rs` constant, and the ops CLI carries a third
(`frontend/scripts/bayla-ladder-ops.mjs:97`). Nothing on chain exposes the rate — the IDL
has no constants and no account stores it — so the card cannot read the truth; it can only
be built to match. A card at one rate in front of a program at the other misquotes every
...
`HzxzfSQ…` still runs the superseded 25% build with no rate guard, so a Preview pointed at
