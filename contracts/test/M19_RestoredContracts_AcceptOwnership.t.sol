...
///         override on the three restored contracts that were missing it
...
///
/// @dev    PROVENANCE 2026-09-10 (T27, branch hygiene): the three overrides this file
///         pins are present on trunk, but the file itself never was — it was authored on
///         `origin/main` in #78 and `main` stopped being merged on 2026-07-23. A
///         content-level diff of `origin/main` against `origin/mvp-launch` found it as
///         one of only two `.t.sol` files that exist on `main` and nowhere on trunk, so
///         the cancel-on-handoff behaviour of these three contracts has been shipping
///         with **zero** test coverage on the branch that actually deploys. Ported here
///         verbatim in behaviour; only this note and the mutation-check record below are
///         new.
///
///         MUTATION-CHECKED against trunk before landing. Deleting the pending-clearing
///         body from each override in turn makes the matching test fail; see the PR body
///         for the exact commands and observed output. The assertions deliberately pin
///         the OBSERVABLE post-handoff state (pending slot zeroed, `_executeAfter`
///         cleared, owner moved), never a function name or a revert string, so a rename
///         of the internal timelock keys does not red this file while the guarantee
///         holds.
