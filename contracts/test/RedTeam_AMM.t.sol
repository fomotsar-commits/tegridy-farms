...
contract ReentrancyAttacker {
    ...
    // Token receive callback would trigger reentrancy Ã¢â‚¬â€ but nonReentrant blocks it
    receive() external payable {}
}
...
contract RedTeamAMM is Test {
    ...
    function test_ATTACK2_feeOnTransferDrainViaRouter() public {
        ...
        // Try to claim more than correctOut Ã¢â‚¬â€ should revert with "K"
        bool success;
    ...
    function test_ATTACK2b_fotRouterSwapAccounting() public {
        ...
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            10 ether,
            0, // no min Ã¢â‚¬â€ just testing accounting
            path,
            attacker,
    ...
    function test_ATTACK4_kInvariantDecrease() public {
        ...
        try pairAB.swap(1 ether, 0, attacker, "") {
            // If this succeeds, K decreased Ã¢â‚¬â€ CRITICAL BUG
        } catch {
            reverted = true;
        }
    ...
    function test_ATTACK5_bypassDisabledPairs() public {
        ...
        // Try to swap through router Ã¢â‚¬â€ should revert
        vm.startPrank(attacker);
        ...
        // The pair itself has no disabled check Ã¢â‚¬â€ only the router does
        ...
        // FINDING: MEDIUM Ã¢â‚¬â€ disabled pair bypass via direct pair interaction
    }
    ...
    function test_ATTACK6a_skimStealsExcessTokens() public {
        ...
        // FINDING: MEDIUM (by design) Ã¢â‚¬â€ permissionless skim() steals direct deposits
    ...
    function test_ATTACK6b_syncManipulatesReserves() public {
        ...
        // This means the price is now skewed Ã¢â‚¬â€ next swapper gets a bad deal
        ...
        // NOTE: The donation is irreversible Ã¢â‚¬â€ the donator loses their tokens
    }
    ...
    function test_ATTACK7b_swapFeeRouterZeroFeeExploit() public {
        ...
        // Try a 9999 wei swap Ã¢â‚¬â€ fee should be 0 but clamped to 1
        bool reverted = false;
    ...
    function test_ATTACK9_sandwichAttack() public {
        ...
        // This is an MEV exploit, not a smart contract bug Ã¢â‚¬â€ but we test if
        ...
        // Alice uses amountOutMin = 0 (no slippage protection Ã¢â‚¬â€ easy target)
        router.swapExactTokensForTokens(
            aliceSwapAmount, 0, pathAtoB, alice, block.timestamp + 30 minutes
        );
        vm.stopPrank();

        // Step 3: Attacker back-runs Ã¢â‚¬â€ sells all B back to A
        vm.startPrank(attacker);
        uint256 attackerABefore = tokenA.balanceOf(attacker);
        ...
        // DEFENDED (by design) Ã¢â‚¬â€ amountOutMin is the protection
    }
    ...
    function test_ATTACK10_wethWrappingExploit() public {
        ...
        // If refund fails, it wraps to WETH Ã¢â‚¬â€ can this be exploited?
    ...
    function test_ATTACK11_reentrancyOnPair() public {
        ...
        // But we can test by calling swap again after a swap Ã¢â‚¬â€ should fail.
        ...
        // Now try to call swap again in the same tx Ã¢â‚¬â€ this is not reentrancy
    ...
    function test_ATTACK12_create2AddressCollision() public {
        ...
        // Attempt to create the same pair again Ã¢â‚¬â€ should revert
        bool reverted = false;
        try factory.createPair(address(tokenA), address(tokenB)) {
            // Should not succeed Ã¢â‚¬â€ pair already exists
        } catch Error(string memory reason) {
            reverted = true;
        ...
        try factory.createPair(address(tokenB), address(tokenA)) {
            // Should also revert Ã¢â‚¬â€ same pair
        } catch Error(string memory reason) {
            reverted = true;
            assertEq(reason, "PAIR_EXISTS");
        }
    ...
    function test_ATTACK13_swapToTokenAddress() public {
        // FRESH-2026 TEST REALIGN: F-31-D â€” the `to != token0/token1` block was
    ...
    function test_ATTACK16_directPairInteraction() public {
        ...
        // This succeeds Ã¢â‚¬â€ no deadline, no slippage check
