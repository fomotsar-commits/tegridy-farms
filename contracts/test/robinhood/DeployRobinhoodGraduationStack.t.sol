...
import {DeployRobinhoodGraduationStackScript} from "../../script/robinhood/DeployRobinhoodGraduationStack.s.sol";
...
/// @title Shape A on 4663 — the graduation-stack deploy script.
...
contract DeployRobinhoodGraduationStackTest is Test {
    DeployRobinhoodGraduationStackScript internal script;
    ...
    // RobinhoodChainConfig constants.
    ...
    function setUp() public {
        ...
        vm.warp(100 days);
        script = new DeployRobinhoodGraduationStackScript();
        multisig = new DummySafe();
        treasury = new DummySafe();
        pauseGuardian = new DummySafe();
    }

    function _cfg() internal view returns (DeployRobinhoodGraduationStackScript.Config memory) {
        return DeployRobinhoodGraduationStackScript.Config({
    ...
    function _run() internal returns (DeployRobinhoodGraduationStackScript.Deployed memory) {
    ...
    function test_RefusesEveryOtherChain_IncludingTheTestnet() public {
        uint256[3] memory wrong = [uint256(1), 8453, 46630];
        for (uint256 i = 0; i < wrong.length; i++) {
            vm.chainId(wrong[i]);
            vm.expectRevert(
                abi.encodeWithSelector(RobinhoodChainConfig.NotRobinhoodChain.selector, wrong[i])
            );
            _run();
        }
    }
    ...
    function test_RefusesAnEOAMultisig() public {
        DeployRobinhoodGraduationStackScript.Config memory cfg = _cfg();
    ...
    function test_RefusesNonDisjointRoles() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RobinhoodChainConfig.RolesNotDisjoint.selector, "MULTISIG", "TREASURY", cfg.multisig
            )
        );
        script.runForTest(cfg, poolManager, positionManager, permit2, airlock);
    }
    ...
    function test_DeploysTheStackFullyWired() public {
        DeployRobinhoodGraduationStackScript.Deployed memory d = _run();
