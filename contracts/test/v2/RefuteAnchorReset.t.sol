...
contract RefuteAnchorResetTest is Test {
    ...
    // Same scenario, asserted the other way, so the size of the leak is on the record.
    function test_EXPLOIT_measureTheLeak() public {
        ...
        vm.prank(alice);
        dist.getReward();
        emit log_named_decimal_uint("pool was owed", owedToPool, 18);
        emit log_named_decimal_uint("alice took    ", alice.balance - before, 18);
        emit log_named_decimal_uint("totalForfeitedToPool", dist.totalForfeitedToPool(), 18);
    }
