...
| 7 | Staking↔liquidity hooks (full-app review): **#1 fee-split routing** (`afterSwap` skim → `distributeFees` redeems claims and routes REAL currency to staker/treasury/POL sinks in one unlock — automates "100% of swap fees to stakers", defaults to 100% POL = prior behavior) + **#2 premium fee discount** in `_getFee` (Gold Card "Reduced Fees"; **trusted-router gated so it can't be spoofed via raw-router hookData**, try/catch-isolated, disabled by default). **23 tests green** (+ anti-spoof, min-fee floor, split conservation). Admin pass-throughs for the new setters + boosted-LP accrual = follow-ups. | ✅ |
