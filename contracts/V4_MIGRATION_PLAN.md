...
| 8 | **#3 Boosted-LP implemented** (owner-directed, ahead of audit): `TegridyBoostedLP` module (Synthetix `rewardPerToken` verbatim from `TegridyLPFarming`, balance = V4 position liquidity × veTOWELI boost) + hook liquidity-callback `onLiquidityChange` notify (override-and-extend, same pool) + admin timelock wiring. **29 tests green.** v1: per-LP aggregate liquidity, full-range, emissions-funded. UNAUDITED. | ✅ |
| 7 | Staking↔liquidity hooks (full-app review): **#1 fee-split routing** (`afterSwap` skim → `distributeFees` redeems claims and routes REAL currency to staker/treasury/POL sinks in one unlock — automates "100% of swap fees to stakers", defaults to 100% POL = prior behavior) + **#2 premium fee discount** in `_getFee` (Gold Card "Reduced Fees"; **trusted-router gated so it can't be spoofed via raw-router hookData**, try/catch-isolated, disabled by default). **25 tests green** (+ anti-spoof, min-fee floor, split conservation, admin timelock flows). Admin timelock pass-throughs for the new setters (discount/split/sinks) **now wired**. Boosted-LP accrual (#3) = design doc `V4_BOOSTED_LP_HOOK_DESIGN.md`. | ✅ |
