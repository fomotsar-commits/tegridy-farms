...
contract TegridyLendingAdmin is OwnableNoRenounce, TimelockAdmin {
    ...
    /// @dev    `ACCEPTED_COLLATERAL_CHANGE` is flushed without the live-cancel budget
    ///         counter (`removalRetryCount` over on the lending contract): the override is
    ///         a one-shot post-handoff sweep, not an operational cancel. Counting it would
    ///         let an outgoing owner exhaust the budget on flagged collections then transfer
    ///         ownership, locking the new owner out of legitimate cancellation.
