...
| `emergencyDisablePair` spends a daily rate-limit slot only when the call actually disables something — the counter and the `disabledPairs[pair] = true` write both sit behind `if (!disabledPairs[pair])` | TF-031: the cap is GLOBAL (3/day across all callers and pairs), so a slot spent is a slot the circuit breaker no longer has. Re-disabling an already-disabled pair changes no state, yet used to consume one — three state-unchanging calls could exhaust the day's budget and leave a real rug undisableable until rollover. Same shape as D-AMM-L2 at `:698-706`, which rejects a same-guardian no-op for the same reason. The H-2 force-cancel deliberately stays OUTSIDE the guard, because a repeat call must still be able to cancel a pending re-enable | `TegridyFactory.t.sol` emergency-disable suite; snapshot re-pinned 2026-09-04 (2 lines, entirely inside Tegridy-only emergency-disable code — touches no V2-audited surface: not the k-invariant, not the 1/6 fee formula, not the `kLast` lifecycle) |
