...
contract DeployDecayingFeeHookScript is Script {
    ...
    function run() external {
        ...
        console2.log("2. Initialize the pool from THIS MULTISIG ITSELF, and send steps 1+2+3");
        console2.log("   as ONE Safe MultiSend batch (delegatecall into MultiSend, plain CALL");
        console2.log("   out to each target, so PoolManager sees msg.sender == the Safe):");
        console2.log("     a) hook.configurePool(key, startFee, baselineFee, decaySeconds)");
        console2.log("     b) poolManager.initialize(key, sqrtPriceX96)");
        console2.log("     c) positionManager.modifyLiquidities(...)  // the launch position");
        console2.log("   ATOMICITY IS THE POINT, NOT CONVENIENCE: configurePool publishes the");
        console2.log("   exact PoolKey in its own calldata, so a separate later initialize is");
        console2.log("   front-runnable in the mempool - and whoever gets there first chooses");
        console2.log("   the opening price and starts the decay clock.");
        console2.log("   PositionManager.multicall CANNOT substitute for this: its");
        console2.log("   initializePool leg arrives with sender == the router.");
