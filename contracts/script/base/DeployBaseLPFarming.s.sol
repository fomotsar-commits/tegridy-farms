...
/// @title  DeployBaseLPFarming — boosted-LP farming on Base, minus the boost.
...
///
/// @dev    Deploy AFTER DeployBaseMVP and after the pair you intend to farm
///         exists on the Base TegridyFactory. STAKING_TOKEN must be that pair.
///
/// @dev    Env vars:
///           REWARD_TOKEN     — the 18-decimal ERC20 that funds emissions (see above)
///           STAKING_TOKEN    — the Base TegridyPair LP token to farm
...
contract DeployBaseLPFarmingScript is Script {
    ...
    function _validate(Config memory cfg) internal view {
        ...
        BaseChainConfig.requireHasCode(cfg.rewardToken, "REWARD_TOKEN");
        ...
        BaseChainConfig.requireHasCode(cfg.factory, "FACTORY");
    ...
    function _printSummary(Config memory cfg, Deployed memory d) internal pure {
        console.log("");
        console.log("=== BASE LP FARMING COMPLETE ===");
