...
/// @title  VerifyBaseLaunchRail — post-ceremony invariant check for the Base
///         launch rail (and, optionally, the LP farm).
/// @notice DeployBaseLaunchRail offers five ownerships two-step; a missed accept
...
contract VerifyBaseLaunchRailScript is Script {
    struct Inventory {
        address treasury;
        address multisig;
    ...
    function check(Inventory memory inv) public view {
        require(block.chainid == BaseChainConfig.CHAIN_ID, "LRV-0: not Base (8453)");
        console.log("LRV-0:  chain is Base (8453) ..................... OK");
        ...
        require(lp.weth() == BaseChainConfig.WETH, "LRV-3a: launchpad WETH != Base WETH9");
        require(lp.sequencerFeed() == inv.sequencerFeed, "LRV-3b: launchpad feed mismatch");
        BaseChainConfig.requireUptimeDialect(lp.sequencerFeed(), "LRV-3c: launchpad feed");
        ...
        require(
            LaunchRugEscrow(payable(inv.rugEscrow)).weth() == BaseChainConfig.WETH,
            "LRV-4: escrow WETH != Base WETH9"
        );
        ...
        console.log("");
        console.log("=== ALL BASE LAUNCH-RAIL INVARIANTS GREEN ===");
    }
