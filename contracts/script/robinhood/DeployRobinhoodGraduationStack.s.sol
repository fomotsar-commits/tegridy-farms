...
/// @title  DeployRobinhoodGraduationStack — Shape A, extended to 4663.
/// @notice Deploys the OWN-VENUE graduation stack on Robinhood Chain: hook admin,
...
///         owner's 2026-08-22 "graduate to us" directive for this chain resolves
///         to the SAME shape because the same facts hold and the substrate
///         exists: the full canonical V4 stack is live on 4663 (see
///         RobinhoodChainConfig's provenance block). Nothing about this chain
///         justifies reviving the rejected V2-pair shape; see the decision doc's
///         4663 addendum.
///
/// @dev    Chain-specific inputs come from RobinhoodChainConfig CONSTANTS
...
contract DeployRobinhoodGraduationStackScript is Script {
    ...
    ///         RobinhoodChainConfig constants and remains the only broadcast path.
    ...
    function _validate(Config memory cfg) internal view {
        ...
        RobinhoodChainConfig.requireDisjoint(cfg.multisig, "MULTISIG", cfg.treasury, "TREASURY");
    ...
    function _deploy(Config memory cfg) internal returns (Deployed memory d) {
        ...
        RobinhoodChainConfig.requireHasCode(RobinhoodChainConfig.UNISWAP_V4_POOL_MANAGER, "POOL_MANAGER");
        RobinhoodChainConfig.requireHasCode(RobinhoodChainConfig.UNISWAP_V4_POSITION_MANAGER, "POSITION_MANAGER");
        RobinhoodChainConfig.requireHasCode(RobinhoodChainConfig.PERMIT2, "PERMIT2");
        RobinhoodChainConfig.requireHasCode(RobinhoodChainConfig.DOPPLER_AIRLOCK, "DOPPLER_AIRLOCK");
        return _deployWith(
            cfg,
            RobinhoodChainConfig.UNISWAP_V4_POOL_MANAGER,
            RobinhoodChainConfig.UNISWAP_V4_POSITION_MANAGER,
            RobinhoodChainConfig.PERMIT2,
            RobinhoodChainConfig.DOPPLER_AIRLOCK,
            CREATE2_DEPLOYER,
    ...
    function _printSummary(Config memory cfg, Deployed memory d) internal pure {
        console.log("");
        console.log("=== ROBINHOOD GRADUATION STACK COMPLETE (Shape A on 4663) ===");
        ...
        console.log("    whitelists the migrator on the 4663 Airlock: setModuleState([migrator],[4]).");
        console.log("    See docs/WHETSTONE_MIGRATOR_PETITION_4663.md. Do NOT set any frontend");
